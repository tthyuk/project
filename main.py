...
def load_data(file_path, file_type):
    """
    지정된 경로에서 데이터를 로드합니다. CSV 또는 Excel 파일을 지원하며,
...
st.markdown("""
이 앱은 사용자가 업로드하는 **행정동별 치킨 점포수 데이터**와 **시간대별 길단위 생활인구 데이터**를 결합하여, 
...
# 1. 점포-행정동 데이터 업로드 섹션
st.sidebar.subheader('1. 점포-행정동 데이터 (예: chicken_stores.csv 또는 .xlsx)')
uploaded_chicken_file = st.sidebar.file_uploader(
    "점포-행정동 파일을 선택하세요", 
    type=['csv', 'xlsx'], 
    key='chicken_upload'
)
...
if uploaded_chicken_file is not None:
    # 업로드된 파일의 확장자를 확인하여 파일 타입을 결정합니다.
...
# 2. 길단위인구-행정동 데이터 업로드 섹션
st.sidebar.subheader('2. 길단위인구-행정동 데이터 (예: population_by_time.csv 또는 .xlsx)')
uploaded_population_file = st.sidebar.file_uploader(
    "길단위인구-행정동 파일을 선택하세요", 
    type=['csv', 'xlsx'], 
